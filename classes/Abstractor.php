...
'additional' => $additional
	...
	return		 $this->arrayCreator(false,true, "webm","90k",$format_code);
			break;
			case '171';
	return		 $this->arrayCreator(false,true, "webm","128k",$format_code);
			break;
			case '140';
	...
	public function refineVideoJSON($json)
	{
		...
		foreach($formats as $format){
			...
			$files[$format->format_id] = array_merge(['url' => $this->URLTweaker($format->url) . "&title={$title} - Haris Naeem Yt Downloader"] , $formatData);
